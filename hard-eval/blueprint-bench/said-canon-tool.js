...
// Recurse subdirs so the wiki is a real library drill-down: project -> example -> file -> section.
// Paths are RELATIVE to DIR (e.g. "examples/01-rest-crud/csharp/InvoiceController.cs").
function walk(dir){
  let out=[];
  for(const e of fs.readdirSync(path.join(DIR,dir),{withFileTypes:true})){
    const rel=dir?path.join(dir,e.name):e.name;
    if(e.isDirectory()){ if(e.name!=='results'&&e.name!=='node_modules') out=out.concat(walk(rel)); }
    else if(/\.(cs|rs|py|ts|java)$/.test(e.name)) out.push(rel.split(path.sep).join('/'));
  }
  return out;
}
const files=walk('').filter(f=>parse(f).length);
