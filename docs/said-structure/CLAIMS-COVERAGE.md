...
|-----|----------|----------------------|
| ~~Code recall does NOT walk the graph~~ **FIXED (Engine A-graph)** | `ask` now follows `call:` edges from each matched symbol | A query landing on a code symbol now pulls in its CONNECTED call-graph — callees (`code_calls`) AND callers (`code_callers`) — as `kind:"code_graph"` candidates (conf 0.92, deterministic AST edges, bounded fan-out, scoped-aware). Proven: query "check_token" returns check_token + its callee lookup_user + its caller validate_session (test_recall_claims_code::ask_walks_the_code_call_graph). The sym→AST→call-graph walk is real. (Note: `build_concept_links` still excludes `Pillar::Code` — code uses `call:` edges, not `link:` concept edges; that's by design.) |
