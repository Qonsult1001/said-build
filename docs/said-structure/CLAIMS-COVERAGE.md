...
|-----|----------|----------------------|
| **Code recall does NOT walk the graph** | `ask` never follows `call:` edges; `build_concept_links` EXCLUDES `Pillar::Code` (said_file.rs:2773); Engine D-2 strips only `link:` not `call:` | Code recall is SILOS today: a query hits a symbol but does NOT pull in its callees/callers or a connected subtree. The "walk sym→AST→call-graph→concept-graph as one tree" is unbuilt. Needs an Engine-E in `ask` that follows `call:`/`call:`-reverse edges from symbol/semantic seeds. |
