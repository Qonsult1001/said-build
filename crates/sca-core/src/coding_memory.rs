...
// Phase prompts (plan/design/code/test/repair) now live in the canonical prompt
// crate `said-prompts` (`said_prompts::coding`), per the .said standard that ALL
// prompts live there and are MCP-exposed. This module keeps only the MEMORY half
// (the iteration template + storage shape). The orchestration crate composes the
// two: said-prompts (the prompt) + this (the recalled memory) → the LLM.
