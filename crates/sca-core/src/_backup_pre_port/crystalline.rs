...
impl CrystallineCore {
    ...
    pub fn add_docs_quantized(
        ...
        // Add doc_ids (skip if already registered by stream_index)
        for id in &ids {
            if !self.doc_ids.contains(id) {
                self.doc_ids.push(id.clone());
            }
        }
        let start_idx = self.doc_ids.len() - ids.len();
    ...
    fn search_full_hybrid_quantized(
        ...
        let rare_words: AHashSet<&str> = q_expanded.iter()
            .filter(|w| *self.word_idf_fast.get(*w).unwrap_or(&0.0) > 2.5)
            .map(|w| w.as_str())
            .collect();

        // Pre-fetch IDF for each query word (avoids HashMap lookup per doc)
        let q_words_with_idf: Vec<(&str, f32, bool)> = q_expanded.iter()
            .map(|w| {
                let idf = *self.word_idf_fast.get(w).unwrap_or(&0.5);
                let is_rare = rare_words.contains(w.as_str());
                (w.as_str(), idf, is_rare)
            })
            .collect();

        // Pre-compute total query IDF (used for alpha calculation)
        let idf_query_sum: f32 = q_words_with_idf.iter().map(|(_, idf, _)| *idf).sum();
        let idf_query_avg: f32 = idf_query_sum / q_expanded.len().max(1) as f32;

        // Dialogue detection
        let is_dialogue_query = idf_query_avg < 2.2 && q_expanded.len() > 4;
        ...
        // Pre-compute phrase windows (allocated ONCE, not per doc)
        ...
        if query_words_raw.len() >= 3 {
            for &(w_size, boost_val) in &[(4usize, 1.25f32), (3, 1.15)] {
                ...
                for window in query_words_raw.windows(w_size) {
                    ...
                    let idf_sum: f32 = clean_words.iter()
                        .map(|w| *self.word_idf_fast.get(w).unwrap_or(&0.5))
                        .sum();
                    let idf_avg = idf_sum / w_size as f32;
                    if idf_avg < 2.2 { continue; }
                    phrase_windows.push(PhraseWindow { clean_words, phrase, idf_avg, boost: boost_val });
                }
            }
        }

        // Pre-compute speaker words for dialogue queries
        let speaker_words: Vec<&str> = if is_dialogue_query {
            query_words_raw.iter()
        ...
        // All reads are &self — thread-safe. No allocations inside the loop.

        let mut results: Vec<(String, f32)> = survivors.par_iter()
            .filter_map(|&(doc_idx, semantic_dist)| {
                let doc_id = self.doc_ids.get(doc_idx)?;
                let doc_words = self.doc_word_sets_fast.get(doc_idx)?;
                ...
                if doc_text.contains(query_lower) {
                    return Some((doc_id.clone(), 1.0f32));
                }

                // 1. SEMANTIC SCORE
                let s_sem = (1.0 - (semantic_dist / max_hamming)).max(0.0) as f32;

                // 2. FUSED OVERLAP + TF-IDF (single pass, no allocation)
                let mut overlap_count = 0u32;
                let mut overlap_idf_sum = 0.0f32;
                let mut entity_score = 0.0f32;
                let mut common_score = 0.0f32;

                let doc_len = doc_words.len() as f32;
                let length_norm = (1.0 - b_param + b_param * (doc_len / avg_doc_len.max(1.0))).max(0.5);

                for &(word, idf, is_rare) in &q_words_with_idf {
                    if doc_words.contains(word) {
                        overlap_count += 1;
                        overlap_idf_sum += idf;
                ...
                // Dynamic alpha
                let alpha = if overlap_count == 0 {
                    1.0
                } else {
                    let idf_overlap_avg = overlap_idf_sum / overlap_count as f32;
                    let base_alpha = (1.0 - idf_overlap_avg / idf_query_avg.max(0.001)).clamp(0.0, 1.0);
                ...
                // Quadratic boost (length normalization already in BM25 TF saturation)
                let boost = 1.0 + token_ratio.powi(2);
                let s_lex = token_ratio * boost;

                // Phrase match boost (uses pre-computed windows, no allocation)
                let mut phrase_match_boost = 1.0f32;
                for pw in &phrase_windows {
                    let has_all = pw.clean_words.iter().all(|w| doc_words.contains(w.as_str()));
                    if !has_all { continue; }
                    // doc_texts_fast already stores lowercase text
                    if doc_text.contains(pw.phrase.as_str()) {
                        phrase_match_boost = pw.boost;
                        break; // Take first (highest window size = highest boost)
                    }
                }
                let s_lex_boosted = s_lex * phrase_match_boost;
                ...
                if is_dialogue_query {
                    for &word in &speaker_words {
                        let word_lower = word.to_lowercase();
                        if doc_text.contains(word) || doc_words.contains(word_lower.as_str()) {
                            final_score *= 1.15;
                            break;
                        }
                    }
                ...
                Some((doc_id.clone(), final_score))
            })
            .collect();

        results.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

        // Span-density reranking on top candidates (already parallel in v1)
    ...
    pub fn search_unified_quantized(
        ...
        // 1. SMART ROUTING (always runs — analyzes query type)
        ...
        // 2. ART PRE-FILTER (always runs — narrows candidates)
        let art_candidates = self.art_prefilter(&q_expanded, query_text);

        // PATH A: PURE LEXICAL (passkeys, codes) — ART-accelerated
        if route == QueryRoute::PureLexical {
            return self.search_pure_lexical_filtered(
                &query_lower, &q_expanded, top_k, art_candidates.as_deref()
            );
        }

        // NO EMBEDDINGS MODE (privacy/streaming — no matrix_quantized)
        ...
        // PATH B: PURE SEMANTIC — ART-accelerated Hamming
        if route == QueryRoute::PureSemantic {
            let survivors = self.hamming_filtered(&q_bits, 5000, art_candidates.as_deref());
            return self.search_pure_semantic_quantized(&survivors, max_hamming, top_k);
        }

        // PATH C: FULL HYBRID — ART-accelerated Hamming + lexical
        let survivors = self.hybrid_filtered(&q_bits, &q_expanded, art_candidates.as_deref());
    ...
    fn analyze_query_quantized(&self, query_text: &str) -> (QueryRoute, Vec<String>, AHashSet<String>, f32, f32, bool, bool) {
        ...
        for word in &q_words {
            ...
            // Try both raw word and stemmed form for vocabulary lookup
            let stemmed = self.stem_word(word);
            let in_idf = self.word_idf_fast.get(word).or_else(|| self.word_idf_fast.get(&stemmed));
            let in_vocab = self.vocabulary_fast.contains(word) || self.vocabulary_fast.contains(&stemmed);
    ...
    pub fn serialize_index(&self, full_text: bool) -> Vec<u8> {
        ...
        for (token_id, doc_ids_set) in &art_items {
            ...
            let indices: Vec<u16> = doc_ids_set.iter()
                .filter_map(|did| self.doc_ids.iter().position(|d| d == did).map(|i| i as u16))
                .collect();
