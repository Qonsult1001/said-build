...
//! structured output. Per spec §9.3.
...
impl OpenAICompatibleProvider {
    pub fn from_config(cfg: &ForgeConfig) -> ForgeResult<Self> {
        let api_key = cfg.llm.api_key.clone().ok_or_else(|| {
            ForgeError::Config("openai-compatible provider requires `api_key`".into())
        })?;
        let base_url = cfg.llm.base_url.clone().ok_or_else(|| {
            ForgeError::Config("openai-compatible provider requires `base_url`".into())
        })?;
        Ok(Self {
            api_key,
            ...
            client: reqwest::Client::builder()
                ...
                .build()
                .map_err(|e| ForgeError::Config(format!("reqwest: {}", e)))?,
        })
    }
    ...
    pub(crate) fn parse_body(
        ...
        let choices = body.get("choices").and_then(|v| v.as_array()).ok_or_else(|| {
            ForgeError::Llm(format!("no choices in response: {}", raw))
        })?;
        let first = choices.first().ok_or_else(|| ForgeError::Llm("empty choices".into()))?;
        let message = first.get("message").ok_or_else(|| ForgeError::Llm("no message".into()))?;
        let content = message.get("content").and_then(|v| v.as_str()).ok_or_else(|| {
            ForgeError::Llm("no message content".into())
        })?;
...
impl LlmProvider for OpenAICompatibleProvider {
    ...
    async fn complete(&self, req: &CompletionRequest) -> ForgeResult<CompletionResponse> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| ForgeError::Http { url: url.clone(), message: e.to_string() })?;
        ...
        if !status.is_success() {
            if status.as_u16() == 401 || status.as_u16() == 403 {
                return Err(ForgeError::Llm(format!("openai-compat auth failure: {}", raw)));
            }
            if status.as_u16() == 429 {
                return Err(ForgeError::Llm(format!("openai-compat rate limit: {}", raw)));
            }
            return Err(ForgeError::Llm(format!("openai-compat HTTP {}: {}", status, raw)));
        }
