...
//! user message. Any test that exercises `generator::generate` uses this to
//! avoid real LLM calls.

use crate::llm::{CompletionRequest, CompletionResponse, LlmCapabilities, LlmFailureClass, LlmProvider, TokenUsage};
        ...
        *self.call_count.lock().unwrap() += 1;
        ...
        if let Some(class) = maybe_fail {
            return Err(match class {
                LlmFailureClass::Auth => ForgeError::Llm("stub auth failure".into()),
                LlmFailureClass::RateLimit => ForgeError::Llm("stub rate limit".into()),
                LlmFailureClass::Network => ForgeError::Http {
                    url: "stub".into(),
                    message: "network".into(),
                },
                LlmFailureClass::ContextExceeded => {
                    ForgeError::ContextExceeded { slug: "stub".into(), estimated_tokens: 300_000 }
                }
                LlmFailureClass::Parse => ForgeError::Llm("stub: malformed".into()),
                LlmFailureClass::Other => ForgeError::Llm("stub: other".into()),
            });
        }
        let canned = {
            let guard = self.canned.lock().unwrap();
            guard.iter().find(|c| req.user.contains(&c.match_user_contains)).cloned()
        };
        let json = canned
            .map(|c| c.response)
            .unwrap_or_else(|| {
